...
Registers Fastify lifecycle hooks: request timing, auth session decoration, temp file management, and `onStart`/`onStop` callbacks.
...
| `onStart` | `() => Promise<void>` | Callback invoked when the server finishes startup |
| `onStop` | `() => Promise<void>` | Callback invoked when the server begins shutdown |
