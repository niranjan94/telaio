...
Every `AppBuilder` and every `TelaioApi` carries an `F` type parameter constrained to `Features`:
...
When you call `createApp()`, `F` starts as `DefaultFeatures` -- every flag is `false`. None of the optional properties exist on the resulting `TelaioApi`. They are literally not part of the type.
...
## TelaioApi conditional intersections

`TelaioApi<F>` uses conditional types to add or omit properties based on the flags in `F`:

```typescript
export type TelaioApi<F extends Features, TSession, TConfig> = {
  fastify: FastifyInstance;
  config: TConfig;
  logger: Logger;
  start: (options?: StartOptions) => Promise<void>;
  stop: () => Promise<void>;
} & (F['database'] extends true ? { pool: import('postgres').Sql; db: Kysely<unknown> } : unknown)
...
const app = await createApp({ config }).buildApi();
// F = DefaultFeatures = { database: false; cache: false; ... }

// TypeScript error: Property 'pool' does not exist on type
// 'TelaioApi<{ database: false; cache: false; queue: false; auth: false; apiDocs: false }, ...>'
console.log(app.pool);
```

Adding `.withDatabase()` before `.buildApi()` resolves the error:
...
console.log(app.pool);  // postgres.Sql -- no error
...
After `.buildApi()`, the resulting `TelaioApi` exposes `auth: { session: TSession }`. More importantly, Fastify request objects gain a typed `maybeAuthSession` property -- your route handlers see the concrete session shape rather than `unknown`.
