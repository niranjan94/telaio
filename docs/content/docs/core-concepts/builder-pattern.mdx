...
description: How createApp() and AppBuilder wire together features, types, and assembly order into TelaioApi or TelaioConsumer.
---

# Builder Pattern

The builder pattern is Telaio's primary composition mechanism. Instead of a single monolithic config object, you chain method calls to opt into features one by one. Each call narrows the TypeScript types so the final `TelaioApi` only exposes what you actually configured.
...
Each method returns a new `AppBuilder` with the updated type parameters. Nothing is constructed until you call `.buildApi()` or `.buildConsumer()`.

| Method | What it does | Type change |
|--------|-------------|-------------|
| `.withDatabase(options?)` | Registers a postgres.js `Sql` instance and Kysely instance | `F & { database: true }` |
...
| `.onStart(fn)` | Registers a hook called inside `start()` | no change |
| `.onStop(fn)` | Registers a hook called inside `stop()` | no change |
...
- `.buildApi()` -- assembles a full Fastify server (`TelaioApi`)
- `.buildConsumer()` -- assembles a lightweight queue worker (`TelaioConsumer`)

Both share the same builder chain. A common pattern is to export a `getBuilder()` function and call the appropriate build method in each entry point:
...
Calling `.buildApi()` resolves to a `TelaioApi<F, TSession, TConfig>`. Some properties are always present; others are conditional on the feature flags baked into `F`.
...
|----------|---------|------|
| `pool` | `database: true` | `import('postgres').Sql` |
...
`.buildApi()` performs a fixed 12-step assembly. The order is not arbitrary -- Fastify's plugin model requires that hooks registered by a plugin only fire for routes registered after it.

1. Create `FastifyInstance`
2. Set up database -- postgres.js `Sql` instance, Kysely instance
...
Some auth libraries (such as better-auth) require a database connection during their own initialization, before you call `.buildApi()`. This creates a circular dependency: you need `pool` to configure auth, but `pool` is normally created inside `.buildApi()`.

Telaio provides standalone factory functions that mirror what `.buildApi()` would create internally. You can pass the pre-built instances back into the builder so the same connections are reused rather than duplicated.
...
// Step 2: pass the sql connection into the auth library
