...
## PostgreSQL + postgres.js + Kysely

PostgreSQL is the database. Not because other databases are bad -- because committing to one lets Telaio optimize for it deeply. The migration system and pg-boss running on the same instance both assume Postgres.

`postgres` (postgres.js) is the Node.js PostgreSQL client. It is fast, modern, and built for ESM. The Kysely integration uses `kysely-postgres-js` as the dialect adapter. Note that `pg-boss` uses `pg` internally for its own connection, which is separate from the application pool.
...
Every heavy dependency in Telaio is an optional peer dependency. `postgres`, `kysely`, `kysely-postgres-js`, `redis`, `pg-boss`, `@aws-sdk/client-s3`, `@aws-sdk/client-ses` -- none are hard dependencies of the package. They are loaded via dynamic `import()` at runtime.
