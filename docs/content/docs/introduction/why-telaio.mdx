...
**No wiring overhead.** A one-liner `createApp().withDatabase().withCache().buildApi()` gives you a fully wired Fastify app with a pg pool, Kysely instance, Redis cache, and all the scaffolding connected. You did not configure any of it. You just used it.
...
createApp()
  ...
  .buildApi()
  // Promise<TelaioApi<{ database: true; cache: true; ... }>>
```

`TelaioApi<F>` uses conditional types to expose only the properties that were enabled:
...
type TelaioApi<F> = {
  ...
  ? { pool: import('postgres').Sql; db: Kysely<DB> }
  : unknown) &
...
If you need to swap PostgreSQL for MySQL or MongoDB, Telaio is the wrong tool. The database layer is built around `postgres.js` and Kysely specifically.
