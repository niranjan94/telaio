...
Telaio's cache module wraps [redis (node-redis)](https://github.com/redis/node-redis) with two important properties: it degrades silently when Redis is unavailable, and it provides a typed record API so you do not have to manually serialize/deserialize JSON.
