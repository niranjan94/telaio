...
better-auth requires a database connection during its own initialization. But under normal usage, Telaio creates the pool inside `.buildApi()` -- which runs after better-auth needs to be initialized.
...
// Step 2: Create better-auth instance using the same sql connection
