...
Consumers run in a separate long-lived process. Use `buildConsumer()` on the builder to create a lightweight queue worker that shares the same wiring as your API server:

```typescript
// src/app.ts -- shared builder
...
`buildConsumer()` creates a `TelaioConsumer` that connects to pg-boss, subscribes each key in the registry to its handler function, and blocks until the process receives `SIGINT` or `SIGTERM`. On shutdown it drains in-flight jobs before exiting.

Run the consumer entry point directly:

```bash
node --import tsx/esm src/consumer.ts
```
...
**SSL:** queue connections follow the same SSL resolution as the main database pool. SSL is auto-enabled for `rds.amazonaws.com` hostnames; `ssl: true` (or `DATABASE_SSL=true` when using a config record) forces it on, and `ssl: false` forces it off.
