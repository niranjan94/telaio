...
// Always run framework migrations first
await runFrameworkMigrations(db);

// Then run your app migrations
await migrateToLatest(db, 'src/db/migrations');
```

<Callout type="warn">
Always run `runFrameworkMigrations` before `migrateToLatest`. Framework migrations set up internal Telaio tables (such as the queue schema) that your app migrations may depend on.
</Callout>
