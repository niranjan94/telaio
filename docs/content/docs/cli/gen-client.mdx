...
The generated client is written to the configured `output` directory. After
generation, `gen-client` runs `biome lint --write` followed by `biome format
--write` over the output directory so the client matches your project's
...
<Callout type="info">
The post-processors pass `--vcs-use-ignore-file=false` so the output directory
is processed even when it is listed in `.gitignore` (typical for generated
code) and your `biome.json` has `vcs.useIgnoreFile: true`.
</Callout>
