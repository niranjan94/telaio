...
<Callout type="info">
`kysely-codegen` uses `pg` internally for database connections. Install it as a dev dependency alongside `kysely-codegen`: `pnpm add -D pg`
</Callout>
...
const app = await createApp({ config }).withDatabase().buildApi();
...
```bash
pnpx telaio db:types --watch
```

When `--watch` is passed, `db:types` watches your migration files for changes and regenerates types automatically after each migration.
