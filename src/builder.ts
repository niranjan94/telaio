...
> {
  ...
  private _onStart: Array<() => Promise<void>> = [];
  private _onStop: Array<() => Promise<void>> = [];
  ...
  /** Register a callback to run when the server starts. Multiple calls register multiple callbacks. */
  onStart(fn: () => Promise<void>): AppBuilder<F, TSession, TConfig> {
    this._onStart.push(fn);
    return this;
  }

  /** Register a callback to run when the server stops. Multiple calls register multiple callbacks. */
  onStop(fn: () => Promise<void>): AppBuilder<F, TSession, TConfig> {
